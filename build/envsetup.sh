...
function breakfast()
{
    target=$1
    local variant=$2
    SPARK_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    for f in `/bin/ls vendor/spark/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function deleteOTA() {
    ...
    curl --header "Authorization: Token $OTA_API_USER_TOKEN" \
        ...
        --request DELETE \
        https://api.spark.org/ota/builds/$1/;
}
...
function pushOTA() {
    ...
    device=$(grep ro\.spark\.device $OUT/system/build.prop | cut -d= -f2);
    file=$(ls -t ${OUT}/spark_$device-10* | sed -n 2p);
    md5=$(md5sum $file | awk '{ print $1 }');
    build_type=$(echo $BUILD_TYPE | tr '[:upper:]' '[:lower:]');
    size=$(stat -c%s $file);
    version=$(grep ro\.spark\.status $OUT/system/build.prop | cut -d= -f2);
    if [ -z $version ]; then
        version=$(grep ro\.spark\.status $OUT/vendor/build.prop | cut -d= -f2)
    fi
    notes=""
    ...
    url="https://sourceforge.net/projects/Spark-Rom/files/$device/${file##*/}";
    if [[ "${TEST_BUILD}" == true ]]; then
        url="https://sourceforge.net/projects/Spark-Rom/files/$device/cucked/${file##*/}";
    fi
    ...
    curl --header "Authorization: Token $OTA_API_USER_TOKEN" \
        ...
        --request POST \
        --data "$data" \
        https://api.spark.org/api/ota/builds/;
}
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/spark/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
