...
- sparkgerrit:   A Git wrapper that fetches/pushes patch from/to sparkOS Gerrit Review.
- sparkrebase:   Rebase a Gerrit change and push it again.
- sparkremote:   Add git remote for sparkOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for sparkOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the spark model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch spark_$target-$variant
        fi
    fi
    return $?
}
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.spark.device | grep -q "$SPARK_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $SPARK_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function sparkremote()
{
    ...
    git remote rm spark 2> /dev/null
    ...
    local SPARK_USER=$(git config --get review.review.spark-os.live.username)
    if [ -z "$SPARK_USER" ]
    then
        git remote add spark ssh://review.spark-os.live:29418/$PFX$PROJECT
    else
        git remote add spark ssh://$SPARK_USER@review.spark-os.live:29418/$PFX$PROJECT
    fi
    echo "Remote 'spark' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/Spark-OS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.spark.device | grep -q "$SPARK_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        sparkremote
        git push spark HEAD:refs/heads/'$1'
    fi
    '
}

function sparkgerrit() {
    ...
    local user=`git config --get review.review.spark-os.live.username`
...
function sparkrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "sparkOS Gerrit Rebase Usage: "
        echo "      sparkrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.spark-os.live/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.spark.device | grep -q "$SPARK_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/spark/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SPARK_FIXUP_COMMON_OUT ]; then
