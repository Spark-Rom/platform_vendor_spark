...
function sparkremote()
{
    ...
    local SPARK_USER=$(git config --get review.review.sparkos.org.username)
    if [ -z "$SPARK_USER" ]
    then
        git remote add spark ssh://review.sparkos.org:29418/$PFX$PROJECT
    else
        git remote add spark ssh://$SPARK_USER@review.sparkos.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/SparkOS/$PROJECT
...
function sparkgerrit() {
    ...
    local user=`git config --get review.review.sparkos.org.username`
...
function sparkrebase() {
    ...
    git fetch "http://review.sparkos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
