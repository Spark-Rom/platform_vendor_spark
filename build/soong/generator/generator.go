...
func init() {
	android.RegisterModuleType("spark_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = sparkExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = sparkExpandVariables(ctx, dep_file)
	...
	cmd := sparkExpandVariables(ctx, String(g.properties.Cmd))
