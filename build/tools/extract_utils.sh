...
function setup_vendor() {
    ...
    if [ ! -d "$SPARK_ROOT" ]; then
        echo "\$SPARK_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SPARK_ROOT/$OUTDIR" ]; then
        mkdir -p "$SPARK_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SPARK_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$SPARK_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$SPARK_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SPARK_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR SPARK\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR SPARK\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR SPARK\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "# Copyright (C) 2017-$YEAR SPARK\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR SPARK\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR SPARK\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SPARK_ROOT"/prebuilts/tools-spark/common/smali/baksmali.jar
        export SMALIJAR="$SPARK_ROOT"/prebuilts/tools-spark/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SPARK_ROOT"/prebuilts/tools-spark/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SPARK_ROOT"/prebuilts/tools-spark/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$SPARK_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SPARK_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$SPARK_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$SPARK_ROOT"/vendor/spark/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SPARK_ROOT"/"$OUTDIR"/radio
